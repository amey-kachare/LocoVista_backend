...
export const createTour = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to create tour', error: err.message });
  }
};
...
export const updateTour=async(req,res)=>{
  ...
  try {
    ...
    res.status(200).json({ success: true, message: 'Tour successfully updated', data: updatedTour });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Failed to update tour', error: err.message });
  }
}
...
export const deleteTour=async(req,res)=>{
    ...
    try {
      ...
      res.status(200).json({ success: true, message: 'Tour successfully deleted', data: deletedTour });
    } catch (err) {
      res.status(500).json({ success: false, message: 'Failed to delete tour', error: err.message });
    }
}
...
export const getAllTour=async(req,res)=>{
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch tours', error: err.message });
      }
}
...
export const getTourBySearch = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Failed to fetch tours based on search', error: err.message });
    }
  };
